...
/** 
 * Transport with socket instead of pipe
 */
class Transport extends EventEmitter {
    ...
    send(message) {
        ...
        this.socket.write(Buffer.from(header.buffer));
        this.socket.write(serializedMessage);
    }

    static pipename( name ) {
        return "\\\\.\\pipe\\" + crypto.createHash('md5').update(name).digest('hex');
    }
}
