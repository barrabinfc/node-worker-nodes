...
function setupModule({ modulePath }) {
    ...
    let socket = net.createServer( function(stream) {        
        $dataTransport = new Transport( stream ); //new net.Socket({ fd: 3 }));
        $dataTransport.on('message', handleCall);    
    
        stream.on('end', _ => stream.close())
    })
    socket.listen( Transport.pipename(modulePath+process.pid) )
